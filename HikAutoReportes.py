...
# Carpetas
INPUT_DIR  = Path("Hikinput")
OUTPUT_DIR = Path("Hikoutput")
...
def procesar_csv(csv_path: Path):
    ...
    if COL_ID in df.columns:
        df[COL_ID] = df[COL_ID].str.lstrip("'").str.strip()
    ...
    for usuario in sorted(df[COL_NOMBRE].dropna().unique()):
        df_usuario = df[df[COL_NOMBRE] == usuario].reset_index(drop=True)
        ws = wb.create_sheet(title=limpiar_nombre_hoja(usuario.strip()))
 
        ws.append(df.columns.tolist())
        for _, fila in df_usuario.iterrows():
            ws.append(fila.tolist())
 
        n_filas = len(df_usuario)
        ...
        print(f"  OK {usuario.strip()} ({n_filas} registros)")
...
def main():
    ...
    if not csvs:
        print(".csv not found in Hikinput folder")
